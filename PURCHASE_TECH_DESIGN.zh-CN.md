...
#### 6.1.2.C 当前采购交易币种口径补充

- ERPNext 采购发票会校验供应商在当前公司下的应付账户币种与业务单据币种是否一致。
- 当前 `create_purchase_order` 不再静默覆盖前端显式传入的采购币种：
  - 若前端传入币种与供应商应付账户币种一致，则按前端币种创建采购单。
  - 若前端传入币种与供应商应付账户币种不一致，且 ERPNext 未启用“允许单一往来账户处理多币种发票”，则后端返回明确业务错误。
  - 若前端未传入币种，则后端优先使用供应商在当前公司下的应付账户币种，再回退到公司默认币种和供应商默认币种。
- `get_supplier_purchase_context` 返回的 `suggestions.currency` 表示当前供应商 + 当前公司组合下建议使用的交易币种。
- `get_purchase_company_context` 返回的 `currency` 表示当前公司默认币种，用于移动端上下文展示和兜底提示。
- 测试阶段建议尽量保持供应商、公司和往来账户主数据币种一致，避免 `_Test Company` 等多币种测试公司混入日常采购流程。
...
- 当前这套能力应明确视为：
  - “来源单据依据版采购退货”
  - 即退货必须基于单一 `Purchase Receipt` 或单一 `Purchase Invoice`
  - 适用于严格可追溯的标准逆向冲销
