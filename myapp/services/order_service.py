...
def _sales_search_batch_size(limit: int, start: int):
...
def _sales_summary_rank(summary_row: dict, sort_by: str):
	...
	if resolved_sort == "unfinished_first":
		return (-_sales_summary_sort_weight(summary_row), modified_ts)
	return None


def _finalize_sales_ranked_page(heap_rows: list[tuple], sort_by: str, start: int, limit: int):
	if not heap_rows:
		return []
...
def _get_latest_payment_entry_summary(invoice_names: list[str]):
	return _build_sales_latest_payment_summary_map({"__single__": invoice_names}).get(
		"__single__",
		_build_empty_sales_payment_summary(),
	)
	...
	try:
		...
		while True:
			...
			for row in summaries:
				sequence += 1
				if _is_sales_summary_cancelled(row):
					cancelled_count += 1

				if not _sales_summary_matches_filter(row, "all", exclude_cancelled=resolved_exclude_cancelled):
					continue

				total_count += 1
				if _is_sales_summary_unfinished(row):
					unfinished_count += 1
				if _is_sales_summary_delivery_pending(row):
					delivery_count += 1
				...
				rank = _sales_summary_rank(row, resolved_sort)
				if rank is None:
					continue
...
def quick_create_order_v2(customer: str, items: list[dict], **kwargs):
	...
	try:
		def _quick_create_order_v2():
			...
			detail = get_sales_order_detail(order_name).get("data", {}) if include_detail and order_name else None
			return {
				"status": "success",
				"order": order_name,
...
def quick_cancel_order_v2(order_name: str, rollback_payment: bool = True, **kwargs):
	...
	try:
		def _quick_cancel_order_v2():
			...
			detail = get_sales_order_detail(order_name).get("data", {}) if include_detail else None
			return {
				"status": "success",
				"order": order_name,
