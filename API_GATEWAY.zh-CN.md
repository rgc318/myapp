...
失败或过期的只读 Agent Run 可由原用户调用 `resume_ai_run_v1(run_id)` 或 `stream_ai_run_resume_v1(run_id)` 恢复。Frappe 在 Run 行锁内验证所有权、活动会话、`agent-state-v1`、原 Prompt 版本、原模型别名和原工具白名单，然后重新置为 `running` 并签发新能力令牌；`completed`、`cancelled`、仍在 `running` 或没有安全检查点的 Run 均拒绝恢复。恢复接口不接受新用户消息，不创建新 Run。
