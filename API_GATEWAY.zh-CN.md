...
契约还保存服务端 `resolved_scope`：商品修改绑定商品编码，销售/采购修改绑定订单号，库存绑定商品、仓库及调整方向。已绑定值不能更换或清空；尚未解析的值允许首次绑定，数量和金额仍按原业务规则编辑。库存当前仅支持单行，额外行明确拒绝，不再静默只取第一行。此快照防止后续目标漂移，不证明模型首次提取忠实于原文；完整目标集合、字段授权和执行计划证明仍待实现。

自动识别入口 `resolve_ai_scenario_v1` 现校验 ai-action-contract-v1：不支持动作、否定/咨询、多目标待确认或写路由缺少契约时返回 VALIDATION_ERROR 和明确说明，不签发可进入写草稿的 resolution_id。已有 Web 使用统一错误展示，不新增页面私有判断。四类草稿直调和执行前已接入上述校验；专门澄清 UI 和完整语义授权仍待实现。
...
- AI 模型管理：`get_ai_model_governance_overview_v1`、`list_ai_audit_events_v1`、`sync_ai_model_registry_v1`、`check_ai_model_availability_v1`、`list_ai_models_v1`、`update_ai_model_registry_v1`、`list_ai_model_policies_v1`、`get_ai_model_policy_v1`、`save_ai_model_policy_draft_v1`、`validate_ai_model_policy_v1`、`approve_ai_model_policy_v1`、`publish_ai_model_policy_v1`、`rollback_ai_model_policy_v1`、`get_ai_model_usage_summary_v1`
  - `update_ai_model_registry_v1` 只维护治理字段：人工 `display_name`、状态、数据区域、留存策略、敏感数据许可、输入/输出成本和币种；供应商能力字段由同步维护。`display_name` 为空时清除人工覆盖，恢复 Backend 自动名称。请求必须包含 `reason` 和幂等键，响应返回递增后的 `registry_version` 与受影响的已发布策略。
...
Agent Runtime 就绪预检按当前场景重新计算有序模型资格，不再要求主模型和全部 fallback 同时保持可用。资格同时检查生命周期、策略能力、工具/JSON Schema 能力和有效健康状态；自动模式只要链中至少一个模型合格即可进入 Agent，返回 `selected_model_alias`、`eligible_model_aliases[]` 和逐模型 `ineligible_models{alias: reasons[]}`。主模型资格漂移时由 Orchestrator 提升第一个合格 fallback，并记录 `fallback_reason=primary_model_ineligible_for_scenario`。显式固定模型不合格时不静默换模，预检返回 `selected_model_ineligible` 并使用兼容查询路径。

失败或过期的只读 Agent Run 可由原用户调用 `resume_ai_run_v1(run_id)` 或 `stream_ai_run_resume_v1(run_id)` 恢复。Frappe 在 Run 行锁内验证所有权、活动会话、`agent-state-v1`、原 Prompt 版本、原模型别名、原工具白名单和持久化 `release_id`，然后重新置为 `running` 并签发新能力令牌；缺少 release ID 的旧 Run 失败关闭。Backend 在同步和 SSE resume 请求中发送 `X-MyApp-AI-Release-Affinity: <run.release_id>`，内部 Router 只路由到该 release 对应的 stable/candidate 池，未知或已退休 release 返回 503，不能随机落到另一 Prompt/runtime revision。`completed`、`cancelled`、仍在 `running` 或没有安全检查点的 Run 均拒绝恢复。恢复接口不接受新用户消息，不创建新 Run。

敏感工具通过 `MyApp AI Agent Approval` 持久审批。Orchestrator 先提交 `waiting_approval` 检查点；内部 `request_ai_agent_tool_approval_v1` 在同一事务中绑定 `run_id + call_id + tool + arguments_hash`、保存裁剪参数摘要、切换 Run 状态并吊销能力令牌。用户可读取单条或分页审批，使用 `review_ai_agent_approval_v1(approval_id, decision, expected_version, reason)` 批准或拒绝；拒绝必须填写原因。审批决定使用乐观版本，重复同决定只返回已记录状态，不能改写已有决定。决定后恢复同一 Run；若审批已提交但恢复调用中断，可使用 `resume_ai_agent_approval_v1(approval_id)` 继续。批准后仍受 `run_id + call_id` 工具幂等保护；拒绝或过期不会执行工具，而是向模型返回结构化拒绝结果。取消 `waiting_approval` Run 会吊销令牌并使未决审批失效。当前三个 Agent 工具均为 `L1_READ_ONLY` 且不触发审批，正式写操作仍走草稿人工确认。
...
会话现在同时维护短期消息历史和受控的 `conversation-state-v2` 工作状态。状态通过 `active_entities` 维护 `product`、`business_document`、`business_partner` 三个 typed slot，可引用商品、销售/采购订单、销售/采购发票、客户和供应商；同时保留订单/报表筛选、结果集 ID 及少量权限过滤后的实体摘要，不保存完整业务结果、模型原文或凭据。唯一查询结果写为 `resolved`，对用户明确提出的新实体查询，多候选写为 `ambiguous`，空结果写为 `not_found`；后两种状态是权威清空信号，不能回退并“复活”旧结果集。仅引用当前 `resolved` 实体的草稿构建失败属于操作失败，不属于一次新的实体解析结果，不得反向把已验证实体降级为空状态。状态由服务端 owner 校验、字段白名单、大小限制和 `state_version` 乐观并发控制保护。每轮业务工具调用前重新读取状态，当前用户明确表达优先于历史状态；业务工具仍必须按当前请求重新执行公司范围、DocType 和记录权限校验。回答成功后才写回状态，版本冲突时跳过写回并审计，不影响当前只读回答。用户清除上下文或状态自动过期时，服务端同时更新 `context_start_sequence`，旧消息仍可在历史中查看但不会继续发送给模型；状态损坏会在下一次发送前恢复为空上下文并记录审计原因。

`auto` 场景统一先调用结构化意图解析器，因此“它”“刚才那个”“这个商品”“这个订单”“这个客户/供应商”“只看未完成的”“换成上个月”等自然语言省略表达可以继承上一轮 typed entity 和筛选。该解析器使用独立的已发布 `intent_parse` 策略，不把仅通过基础 Chat 探测的用户固定模型误用于严格 JSON 路由；固定模型仍写入一次性凭据指纹并用于后续业务回答或草稿资格校验。Web 前置识别成功后，Backend 把解析结果保存在短期服务端缓存，只向浏览器返回 opaque `resolution_id`；正式 Chat/SSE 仅在当前用户、规范化内容、公司、会话 ID、`conversation-state-v2` 版本、附件 ID 和固定模型全部一致时一次性复用。凭据过期、重复使用、字段被改动或上下文版本变化时自动回退正常解析，Backend 不接受浏览器提交可篡改的 intent JSON。Orchestrator 的意图 Prompt 版本以当前 Runtime Manifest 为准；商品查询同时返回核心词、明确属性线索和未确认身份假设，避免把用户整句话硬编码成唯一搜索词。语义路由优先决定当前场景，本地规则只在模型不可用、低置信度、输出非法，或需要防止明确写操作被降级到只读 Agent 路径时兜底。状态只用于解析辅助，不能把历史业务事实当作实时事实，也不能替代本轮 Frappe 查询。
...
`stream_ai_message_v1` 和四类 `generate_ai_*_draft_v1` 草稿端点都接受可选 `retry_run_id`，用于消息级原位重试。该参数只能引用当前账号、当前活跃会话中最后一条失败或取消的助手 Run，且草稿重试必须与原业务场景一致；Backend 从持久化记录恢复原用户问题、场景、公司、会话和附件编号，不信任浏览器重新拼装这些事实。重试时不再插入重复用户消息，而是创建新的 Run，将原失败助手消息重新绑定到新 Run，并在成功后原位更新正文和 citation。旧 Run 保留不变，新 Run 通过 `retry_of_run_id` 记录来源。草稿生成失败必须与普通 Chat 一样持久化绑定失败 Run 的助手占位；实现顺序为回滚未完成草稿、标记 Run 失败、写入失败占位并提交，不能只留下孤立 Run。
...
四类草稿的目标身份和 provenance 由 Backend 统一治理。服务端在模型输出标准化后绑定的商品、订单、客户或供应商目标属于本轮权威目标，重复标准化不得覆盖；旧 `_state.entity` 只能在没有本轮目标时作为同一草稿的兼容基线。`target_source / target_context_ref` 不属于客户端可编辑契约：`update_ai_draft_v1` 会忽略浏览器回传值，只在商品编码、订单号、客户/供应商或订单行稳定身份未改变时从当前持久化版本恢复。订单行重排按 `row_id`、唯一来源哈希或唯一商品身份匹配，不按数组下标继承；无法唯一匹配时来源降级为 `explicit_or_model_query`。切换订单号同时清除旧 `source_order_modified`，执行前重建新目标快照并通过版本冲突要求再次复核。

商品草稿类型为 `product_setup`，`operation` 为 `create` 或 `update`。创建模式承载 Item 主数据、商品昵称、商品图片、条码、规格、四类价格和可选初始库存，复用幂等 `create_product_v2`；完善模式读取现有商品、昵称、图片和价格作为 baseline，只把 `_state.patch` 交给 `update_product_v2`。`nickname` 是内部称呼、俗称或区分性简称，不替代正式 `item_name`；明确清空时进入 `_state.patch.nickname=null`，执行层以空字符串写回正式商品。AI 来源图片通过私有短期 Attachment 发送给经过视觉探测的模型；只有明确创建且草稿未指定图片时，第一张来源图片才派生暂存封面。完善现有商品绝不自动覆盖图片。完善模式不接受初始库存，当前库存仅在 `_state.context` 中只读展示；库存变化必须使用库存调整。旧草稿的 `valuation_rate` 仅作为兼容输入读取。用户可在 AI 工作台确认当前版本后原地执行，也可选择进入 `/master-data/products` 处理复杂字段。

商品完善命令中的 `target.item_code / target.barcode / target.query / target.context_ref` 只描述修改前的目标，`patch` 新值不得参与目标搜索。`target.query` 统一进入共享商品实体解析器，Backend 始终把 `nickname` 加入检索字段，即使模型只请求按正式名称搜索也不能丢失昵称召回；只有解析器返回权限范围内的确定性 `selected`（稳定编码、条码、唯一精确名称或标准昵称）时才自动绑定 Item。候选响应在有值时返回 `nickname`，供 Web 明确区分相似商品。仅有一个模糊/语义候选但没有 `selected` 时仍要求人工确认，多候选和零候选同样失败关闭。验证契约分别返回 `PRODUCT_TARGET_CONFIRMATION_REQUIRED`、`PRODUCT_TARGET_AMBIGUOUS`、`PRODUCT_TARGET_NOT_FOUND`，字段固定为 `target.item_code`；不得把“已找到候选待确认”误报为“未找到”。正式写入仍必须经过草稿复核、版本校验和用户确认。

商品查询后的指代表达使用服务端受控业务上下文，不依赖模型从助手文字猜测商品。`generate_ai_product_setup_draft_v1` 在本轮未提供 `item_code`，且用户明确表达修改/完善意图并使用“这个商品、刚才查询到的商品、它”等指代时，只允许继承当前有效会话中 `resolution_status=resolved` 的唯一商品，或最近商品结果集中唯一的 `entity_id`；即使结构化模型把操作降级为 `auto`，Backend 也只在该明确更新条件下纠正为 `update`。显式商品编码优先；多个候选、过期或已清除上下文、非商品结果集均不自动绑定。服务端完成上下文绑定后，后续命令标准化必须保持幂等，不能再用模型原始的空 `target.item_code` 覆盖服务端 `_target_item_code`。绑定后 Backend 仍按当前用户、公司和 Item 权限重新读取正式商品，并把不可变目标写入 `_state.entity`；Run 工具审计记录目标编码和来源。该规则同样适用于库存、销售和采购草稿中的活动商品，以及销售/采购的活动订单和客户/供应商。若上下文绑定草稿因其他校验失败，失败草稿不会清除对应原 `resolved` 实体；用户明确切换到新实体、清除上下文或状态过期时仍按失败关闭规则处理。
...
调整后库存高于当前库存时，服务端按“已核验的用户选择或人工输入 → 当前仓库有效库存估值 → 所选单位唯一有效采购价候选”解析最终按库存基准单位计价的 `valuation_rate`。所有当前有效的采购型 `Item Price` 都作为 `valuation_rate_candidates[]` 返回，不限于 `Standard Buying`；每项包含稳定 `reference_id`、价格表、币种、原价格、计价单位、换算系数、折算后库存单位价格、有效期和是否可用。箱价、件价等不同包装单位是独立的正常价格，不因折算结果不同而判定为冲突；当前估值为零时，只有所选调整单位恰好存在一个可用候选才自动采用，多个同单位候选或仅有其他单位候选都要求用户明确选择或人工填写。币种与公司库存估值币种不同、缺少计价单位或缺少商品换算关系的候选不可采用。

采用采购价时提交 `valuation_rate_source=buying_price_reference` 与 `valuation_rate_reference_id`；Backend 必须按 Item Price ID 重新读取并校验商品、有效期、币种和单位换算，忽略客户端伪造的折算结果。人工填写使用 `valuation_input_rate + valuation_input_uom` 表达所选计价单位价格，由 Backend 折算出库存基准单位 `valuation_rate`；旧 `standard_buying_reference` 和直接提交 `valuation_rate` 的草稿仅作兼容读取。采购价格表本身只是估值候选，只有被采用并折算为正式 `valuation_rate` 后才影响库存价值；库存调整不会创建采购订单、采购发票或供应商应付。增加库存而最终估值仍无有效正数时草稿不得标记为可执行；减少库存不要求填写新的计价单位价格。两类卡片动作必须使用 POST 与 `Idempotency-Key`，同时按“用户 + 会话 + 公司 + 动作 + 当前有效商品”执行业务幂等：同一活动草稿首次返回 `outcome=created`，后续即使使用不同 request id 也返回同一草稿并标记 `outcome=reused`；复用历史库存草稿时会重新读取实时 Bin、采购价候选和价值影响并刷新版本，同时保留仍可验证的用户选择或人工输入。草稿执行、交接或放弃后释放业务键，允许重新创建。直接 UI 动作返回 `messages=[]`，不伪造用户/助手聊天，也不进入后续模型上下文。库存草稿存在候选商品时，`select_ai_draft_product_candidate_v1(draft_id, expected_version, item_code, selection_text)` 只接受当前草稿候选列表内的编码，保留原增减方式、数量、单位、仓库、原因和日期，更新同一个草稿版本，不重新执行商品搜索、模型调用或追加聊天消息。
...
图片中明确出现的订单行价格属于用户提供的业务输入。首次生成草稿时 Backend 允许采用该价格，保存 `price_source=user` 和对应字段 patch，同时保留当前后端参考价；图片未出现价格时继续使用参考价或留空。无论来源如何，正式执行仍要求用户检查并确认当前草稿版本。
...
- 销售订单、发货单、销售发票、采购订单、采购收货、采购发票及其取消/回退接口在读取或修改具体单据前执行 Frappe 文档级 `read / write / cancel` 权限检查；知道单号不代表有权访问。
- `search_sales_orders_v2`、`get_sales_order_status_summary`、`search_purchase_orders_v2` 和 `get_purchase_order_status_summary` 使用带当前用户权限条件的列表查询，工作台不能绕过公司、User Permission 或记录权限看到其他业务数据。
- 创建、更新、提交、取消、付款、退款、退货、库存调整、主数据维护、用户与权限管理、文件上传、打印任务及 AI 会话写入等接口必须使用 POST。GET 只用于查询与下载类接口，调用方不得用链接跳转或查询参数触发业务写入。
- 商品昵称/规格回填脚本的 HTTP 入口仅允许 `System Manager` 通过 POST 调用；命令行维护继续使用内部执行函数，不开放普通登录用户批量写商品。

### 幂等语义

已绑定站点的带键请求必须使用持久幂等表；表缺失或表状态查询失败时，返回 HTTP 503 / `IDEMPOTENCY_STORE_UNAVAILABLE`，不读取缓存重放、不进入业务回调、不降级到文件锁。表缺失需管理员完成正常站点迁移，临时数据库故障恢复后可用原 key 重试。无 key 请求的既有行为不变；文件锁兼容路径仅留给未绑定站点的测试上下文。

2026-09-13 事务与隔离约束：Gateway 捕获异常并转为错误包络前必须执行数据库 rollback；仅设置 4xx/5xx 不能阻止 Frappe 对 POST 的正常返回执行 commit。rollback 自身失败时必须继续抛出异常。

复合操作的嵌套 `run_idempotent` 共享最外层事务和回执，子调用不得独立抢占、缓存或提交；即使父回调捕获了子调用异常，外层仍按失败处理。外层未传幂等键时，子调用也不能提前提交。该约束不涵盖回调内绕过幂等工具的显式 commit，新增编排仍需审查这些边界。

持久记录保留既有 `(namespace, request_id)` 唯一键并校验 owner：原用户可重放升级前的记录，其他用户使用相同 key 返回 `409 IDEMPOTENCY_KEY_CONFLICT`，必须生成新 key。缓存升级为按用户散列隔离的 v2 key，不读取旧共享缓存。此项不等同于对原用户后续权限撤销进行全面重验。
...
幂等表由 `myapp.tasks.cleanup_idempotency_records` 每小时清理一次，只删除已经过期的最终状态记录，包括 `succeeded`、`failed`、`retryable_failed`。`processing` 使用独立的 15 分钟执行租约，不由清理任务直接删除；请求在租约过期后可以原子接管并重新进入执行，避免进程退出后永久卡在“处理中”，同时避免正常的较慢交易被过早重复执行。业务写入与数据库幂等成功状态在同一事务中提交，再以 Redis 作为非关键缓存，因此缓存不可用不会把已经完成的业务请求误报为失败。
