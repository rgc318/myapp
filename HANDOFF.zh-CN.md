...
- 当前采购补充约定：
  - 商品主数据当前只维护一个默认采购价口径：
  - `Standard Buying`
  - 当前不按“批发采购价 / 零售采购价”拆成多套采购价格体系
  - 采购单前端默认应按商品库存基准单位（`stock_uom`）带入采购单位
  - 若业务上需要按箱、包等业务单位采购，仍允许在订单行改写 `uom`
  - 数量录入后应继续按既有 `uom_conversions` 换算到库存基准单位
  - 因此前端展示“参考采购价”时，默认应理解为商品级默认采购参考价，而不是订单行最终成交价
  - 采购单移动端当前已将“默认入库仓”明确为采购单级上下文，不再放到商品搜索页内做独立切换
  - 默认入库仓优先级已调整为：
    - 当前公司默认仓
    - 若无当前公司默认仓，再回退到供应商建议仓
    - 若操作员已手动修改，则以后续手动结果为准
  - 后端已补充采购公司上下文读取能力：
    - `get_purchase_company_context(company)`
    - 网关入口：`myapp.api.gateway.get_purchase_company_context`
    - 用于返回当前公司口径下的默认仓
    - 该接口的职责是“按公司取采购默认仓”，不是“按供应商给建议仓”
    - 前端当前用于：
      - 采购创建页初始化默认入库仓
      - 采购编辑页补齐空仓行
- 当前新增约定：
...
因此，后续不建议立即继续重投入“高级退货中心”建设；若未来退货优先级提升，更合理的方向应是拆成两类能力：

- 来源单据型退货
- 现场混合 / 自由退货

在此之前，当前退货能力保留、可用、可继续联调，但作为次优先级模块处理。
