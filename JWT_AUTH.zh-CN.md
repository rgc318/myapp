...
2026-09-13：JWT 登录使用 Frappe `get_login_attempt_tracker` 和共享 Redis 失败计数，遵守系统设置 `allow_consecutive_login_attempts` / `allow_login_after_fail`。IP 在密码验证前检查；账号使用 Frappe 返回的规范 User.name，使邮箱/用户名别名共享限制。错误密码、禁用账号与错误 OTP 计入对应账号及 IP；未知账号和超长密码计入 IP。密码最大长度与框架一致。框架 `SecurityException` 默认没有 HTTP 状态，JWT 入口为锁定异常显式设置 HTTP 429，避免误报 500；普通认证失败仍为 HTTP 401。
