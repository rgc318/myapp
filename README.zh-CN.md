...
- AI Copilot 模块设计文档：`AI_TECH_DESIGN.zh-CN.md`
- AI 模型治理与策略管理设计：`AI_MODEL_GOVERNANCE_TECH_DESIGN.zh-CN.md`
- AI 高并发与性能设计：`AI_HIGH_CONCURRENCY_TECH_DESIGN.zh-CN.md`
...
- 若传入 `warehouse + warehouse_stock_qty = 0`，后端也会为该 `item_code + warehouse` 创建正式 `Bin` 库存记录
  - 这表示“已有库存记录、当前为 0”，而不是“没有库存记录”
  - 该行为同时适用于新建商品和编辑商品时切换到新仓库的场景
...
- 商品搜索默认只返回启用商品：
  ...
  - Web 企业级选品可传 `item_group`、`brand` 和 `in_stock_only` 缩小候选商品范围
