...
四类草稿还统一遵守上下文目标不变量：所有 Semantic normalizer 对已经标准化的命令必须幂等；本轮由服务端绑定的商品、订单和客户/供应商目标优先于模型原始空目标与旧草稿 `_state`；旧快照只用于同一目标的 baseline、用户 patch、来源哈希和并发校验。`target_source / target_context_ref` 属于服务端 provenance，浏览器编辑时即使回传也必须先剥离，仅在目标身份未改变时从已持久化草稿恢复。订单行使用 `row_id`、唯一 `source_hash` 或唯一商品身份匹配旧行，不按数组下标继承；重复商品又没有稳定身份时宁可放弃 provenance，也不能串行。订单号改变时同步丢弃旧 `source_order_modified`，执行前重新获取新目标快照并要求用户复核。引用当前 `resolved` 实体的操作失败不清空活动实体；用户明确查询或切换到新实体所得的 `ambiguous / not_found`、手工清除和状态过期仍是权威清空信号。
...
固定评测集采用纯合成用例和确定性 grader，覆盖四类结构化草稿、grounding、无上下文事实边界、Prompt Injection、写操作诱导和系统提示/密钥提取。Offline replay 与真实模型 live gate 均需满足：critical、安全、Schema 和禁止模式 100%，结构化字段准确率不低于 95%，普通场景通过率不低于 90%。只有覆盖当前 mode 全部用例的报告具备 `release_gate_eligible=true`；`--case` / `--tag` 子集报告只用于诊断。当前受控业务查询 Prompt registry 的有效版本为 `erp-readonly-v11`，意图解析为 `erp-intent-v6`，四类草稿分别为 `sales-order-draft-v5`、`purchase-order-draft-v5`、`inventory-adjustment-draft-v3`、`product-setup-draft-v7`，Frappe 审计和 Orchestrator/Langfuse 必须保持同值。语义命令采用 target 与 patch 分离、订单行级 patch、库存调整类型可空；订单表头明确清空通过稳定 clear marker 贯通草稿、保存和执行，替换商品或单位时不得沿用旧计价基础。Backend 不得通过新值、关键词或正则重新猜目标。调用方显式提供不一致或空白 Prompt 版本时，Orchestrator 返回 HTTP `409`；`/health` 返回完整 `prompt_versions`，不得静默覆盖版本漂移。四类草稿的上下文 provenance 与状态投影不变量已在 Backend 统一；完整统一 Entity Resolver 和库存 `target + patch` Schema 迁移仍属于后续工作，不得误报为已完成。
