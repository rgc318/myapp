...
> 状态：Phase A 第二条纵向链路已完成。当前已具备会话/消息/Run 持久化、30 天默认保留期、POST + JWT SSE、Web 增量渲染、Run 反馈、第一个 `search_products` 工具和受限自然语言订单查询 DSL。真实业务工具由 Frappe 在当前用户与公司范围内执行并裁剪后交给 AI Orchestrator，模型不能回调任意接口。语义向量检索、报表工具、Langfuse 和单据草稿仍按本文分期继续实施。
